define(['angular', './sample-module'], function (angular, controllers) {
    ...
    controllers.controller('DashboardsCtrl', ['$state','$timeout','$interval','$scope','$rootScope','$http','$log', 'PredixAssetService', 'PredixViewService','AuthService','HygieneService', function ($state,$timeout,$interval,$scope,$rootScope,$http,$log, PredixAssetService, PredixViewService,AuthService,HygieneService) {
		...
		var loadData = function(token){
		    ...
		    method: 'GET',
		    url: 'https://ehs-rmd-datasource2.run.aws-usw02-pr.ice.predix.io/services/geography/appDashBoard',
		  ...
		  //console.log($scope.SMT_maxaqi);
		  loadChart('#camp31', 0.0, 200.0, data.body[0].maxAqi.aqiValue);
		  loadChart('#camp32', 0.0, 200.0, data.body[1].maxAqi.aqiValue);
		  loadChart('#camp33', 0.0, 200.0, data.body[2].maxAqi.aqiValue);
		 
		  $scope.SMT_updated_status= getStatus(data.body[0].maxAqi.name, data.body[0].maxAqi.aqiValue);
		  $scope.PGF_updated_status= getStatus(data.body[1].maxAqi.name, data.body[1].maxAqi.aqiValue);
		  $scope.NSM_updated_status= getStatus(data.body[2].maxAqi.name, data.body[2].maxAqi.aqiValue);
		  
		  
		  $scope.HM_no2 = data.body[3].attributes[0].NO2
		  $scope.HM_so2 = data.body[3].attributes[1].SO2
		  $scope.HM_pm2_5 = data.body[3].attributes[2].PM2_5
		  
		  $scope.SM_no2 = data.body[4].attributes[0].NO2
		  $scope.SM_so2 = data.body[4].attributes[1].SO2
		  $scope.SM_pm2_5 = data.body[4].attributes[2].PM2_5
		  
		  $scope.Ro_no2 = data.body[5].attributes[0].NO2
		  $scope.Ro_so2 = data.body[5].attributes[1].SO2
		  $scope.Ro_pm2_5 = data.body[5].attributes[2].PM2_5
		  ...
		  if($scope.HM_no2 > 80 || $scope.HM_so2 > 80||$scope.HM_pm2_5 > 60){
				
				$scope.war = true;
			}
			
			else if($scope.SM_no2 >80 || $scope.SM_so2 >80||$scope.SM_pm2_5>60){
				
				$scope.war = true;
			}
			else if($scope.Ro_no2 >80 || $scope.Ro_so2 >80||$scope.Ro_pm2_5>60){
				
				$scope.war = true;
			}
			else if($scope.Wsm_no2 >80 || $scope.Wsm_so2 >80||$scope.Wsm_pm2_5>60){
  	...
  	var getValues = function() {
		...
		HygieneService.dashboardValues2().success(function(data) {
			...
			$scope.smt1_temp = data.body[0].temperature;
			$scope.smt1_nois = data.body[0].noise;
			$scope.smt1_hums = data.body[0].humidity;
			
			$scope.smt2_temp = data.body[1].temperature;
			$scope.smt2_nois = data.body[1].noise;
			$scope.smt2_hums = data.body[1].humidity;
			
			$scope.Hpg_temp = data.body[2].temperature;
			$scope.Hpg_nois = data.body[2].noise;
			$scope.Hpg_hums = data.body[2].humidity;
			
			if(($scope.smt1_temp >= 20 && $scope.smt1_temp <= 40) &&  ($scope.smt1_hums >= 20 && $scope.smt1_hums <= 60) && ($scope.smt1_nois <= 75)){
				$scope.hygn_sat= true;
				//console.log("value: " +$scope.hygiene_satsifactory);
			}
			else
				{
		         $scope.hygn_wa= true;
			...
			if(($scope.smt2_temp >= 20 && $scope.smt2_temp <= 40) &&  ($scope.smt2_hums >= 20 && $scope.smt2_hums <= 60) && ($scope.smt2_nois <= 75)){
				$scope.hygn_sat= true;
				//console.log("value: " +$scope.hygiene_satsifactory);
			}
			else
			...
			if(($scope.Hpg_temp >= 20 && $scope.Hpg_temp <= 40) &&  ($scope.Hpg_hums >= 20 && $scope.Hpg_hums <= 60) && ($scope.Hpg_nois <= 75)){
				$scope.hygn_sat= true;
				//console.log("value: " +$scope.hygiene_satsifactory);
			}
			else
	...
	$scope.gotoaqi = function(assetName){
		...
		$state.go('airquality',{'smtare':assetName});
	...
	$scope.gotohygn = function(assetName){
		...
		$state.go('industrial-hygiene',{'smtare':assetName});
